...
export async function generateMetadata({ params, searchParams }: { params: Promise<{ name: string }>; searchParams: Promise<{ page?: string }> }): Promise<Metadata> {
  ...
  return pageMetadata({ title: `${actressName}のおすすめ作品・セール・埋もれ名作BEST10${page > 1 ? ` ${page}ページ目` : ""} | 発掘LAB`, description: `${actressName}のおすすめ作品、セール中作品、埋もれ名作をBEST10形式で比較。発掘スコア、埋もれ度、レビュー件数、価格条件から選べます。`, canonical: `/actress/${encodeURIComponent(actressName)}${page > 1 ? `?page=${page}` : ""}`, robots });
}
