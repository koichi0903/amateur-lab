...
export async function generateMetadata({ params, searchParams }: { params: Promise<{ name: string }>; searchParams: Promise<{ page?: string }> }): Promise<Metadata> {
  ...
  return pageMetadata({ title: `${actressName}のおすすめ作品・セール・埋もれ名作BEST10${page > 1 ? ` ${page}ページ目` : ""} | 発掘LAB`, description: `${actressName}のおすすめ作品、セール中作品、埋もれ名作をBEST10形式で比較。発掘指数、買い時スコア、レビュー件数、価格条件から選べます。`, canonical: `/actress/${encodeURIComponent(actressName)}${page > 1 ? `?page=${page}` : ""}`, robots });
}
...
function Best10Card({ work, rank, scoreLabel }: { work: ActressBest10Item; rank: number; scoreLabel: string }) {
  return (
...
function Best10Section({ id, title, eyebrow, description, items, icon: Icon, scoreLabel }: { id: string; title: string; eyebrow: string; description: string; items: ActressBest10Item[]; icon: typeof Trophy; scoreLabel: string }) {
  ...
  return (
    <section id={id} className="mt-10 scroll-mt-24">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        {items.map((work, index) => <Best10Card key={work.id} work={work} rank={index + 1} scoreLabel={scoreLabel} />)}
      </div>
    </section>
  );
}
