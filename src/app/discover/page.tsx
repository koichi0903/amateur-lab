...
import { buildInsightsForWorks } from "@/lib/getHomePriceInsights";
...
function saleDetails(work: AiDiscovery) {
  const price = currentPrice(work);
  const isSale = work.is_on_sale || work.sale_price > 0 || work.discount_rate > 0;
  const regularPrice = isSale && work.list_price != null && work.list_price > price ? work.list_price : null;
  const discountRate = isSale && work.discount_rate > 0
    ? Math.round(work.discount_rate)
    : work.sale_price > 0 && regularPrice && price > 0
      ? Math.round((1 - price / regularPrice) * 100)
      : 0;
  return { price, isSale, regularPrice, discountRate };
}
...
export default async function DiscoverPage() {
  ...
  const priceInsights = await buildInsightsForWorks(
    works.slice(0, 40) as unknown as Parameters<typeof buildInsightsForWorks>[0],
    since,
    { requireBuyTimingSignal: false },
  );
  const priceInsightsById = new Map(priceInsights.map((insight) => [insight.id, insight]));
  return <><Header /><main className="min-h-screen bg-[#f8fafc] text-slate-950">
    ...
    <div className="mx-auto max-w-[1500px] px-4 py-10 sm:px-6 lg:px-8"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{works.slice(0, 40).map((work) => { const style = styles[work.reasonType]; const sale = saleDetails(work); const insight = priceInsightsById.get(work.id); return <Link key={work.id} href={workDetailHref(work.id, "home")} className="group rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition hover:-translate-y-1 hover:border-pink-200 hover:shadow-lg"><div className="relative aspect-[4/3] overflow-hidden rounded-xl bg-slate-100"><WorkImage src={work.image_url} alt={work.title} sizes="(max-width: 640px) 92vw, 25vw" className="object-cover transition duration-300 group-hover:scale-105" /></div><div className={`mt-3 inline-flex rounded-full px-2.5 py-1 text-[11px] font-black ${style.badge}`}>{style.label}</div><p className="mt-2 line-clamp-2 min-h-10 text-sm font-black leading-5">{work.reason}</p><h2 className="mt-2 line-clamp-2 min-h-10 text-sm font-black leading-5">{work.title}</h2><div className="mt-3 border-t border-slate-100 pt-3">{sale.regularPrice && <span className="mr-2 text-xs text-slate-400 line-through">通常価格 ¥{sale.regularPrice.toLocaleString("ja-JP")}</span>}<span className={`text-sm font-black ${style.accent}`}>¥{sale.price.toLocaleString("ja-JP")}</span>{sale.discountRate > 0 && <span className="ml-2 rounded bg-pink-50 px-1.5 py-0.5 text-[10px] font-black text-pink-600">{sale.discountRate}%OFF</span>}</div>{insight && <div className="mt-3 rounded-lg border border-slate-100 bg-slate-50 px-2 py-1"><MiniPriceHistoryChart points={insight.priceHistory} windowStartAt={insight.priceWindowStartAt} windowEndAt={insight.priceWindowEndAt} lowPrice={insight.low90Price} currentPrice={insight.currentPrice} variant="compact" /></div>}</Link>; })}</div></div>
  </main></>;
}
