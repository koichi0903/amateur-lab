...
assert.equal(completeThreadEvidence({ sourceAuthorHandle: "creator", parent, ownReplies: [ownReply], fullyObserved: false }).status, "FOUND_COMPLETE_THREAD");
...
assert.equal(completeThreadEvidence({ sourceAuthorHandle: "creator", parent, ownReplies: [], fullyObserved: false }).status, "THREAD_NOT_FULLY_OBSERVED");
