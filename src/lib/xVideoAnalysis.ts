...
function parsePpm(buffer: Buffer): PixelStats {
  ...
  let r = 0, g = 0, b = 0; const histogram = Array.from({ length: 8 }, () => 0); let brightness = 0;
  ...
  return { brightness: brightness / pixels, red: r / pixels, green: g / pixels, blue: b / pixels, histogram: histogram.map((x) => x / pixels) };
}
...
export async function analyzeSampleMovie(input: { sourceUrl: string; trimStartSeconds?: number; jacketUrl?: string | null }): Promise<VideoAnalysisResult> {
  ...
  try {
    ...
    if (black) evidence.push({ kind: "title_card_duration_sec", value: firstChange ?? 0, confidence: firstChange !== null ? 0.78 : 0.55, timeSec: firstChange ?? 0, safePhrase: "冒頭に暗いフレームがある。" });
