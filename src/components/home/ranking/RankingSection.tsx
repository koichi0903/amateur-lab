        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {works.map((work, index) => (
            <Link key={work.id} href={`/works/${work.id}`} className="group flex min-w-0 flex-col rounded-2xl border border-slate-200 bg-white p-2.5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg sm:p-3">
              ...
              <p className="mt-3 whitespace-nowrap border-t border-slate-100 pt-2 text-xs font-black sm:text-sm">{(work.sale_price > 0 ? work.sale_price : work.price) > 0 ? `¥${(work.sale_price > 0 ? work.sale_price : work.price).toLocaleString("ja-JP")}` : "価格未取得"}</p>
            </Link>
          ))}
        </div>
      ) : (
