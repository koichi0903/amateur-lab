    ...
    <section className="mx-auto mt-14 max-w-[1500px] px-4 sm:px-6 lg:px-8">
      <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 shadow-sm sm:p-6">
        ...
        <div className="space-y-7">
          {buyTiming.length > 0 && <Row icon={Crown} eyebrow="BUY TIMING" title="今日の買い時ランキング" href="/ranking"><div className="grid gap-3 overflow-x-auto pb-1 sm:grid-cols-2 lg:grid-cols-3">{buyTiming.slice(0, 3).map((work, index) => <MainBuyCard key={work.id} work={work} rank={index + 1} />)}</div></Row>}
