...
const origin = fs.readFileSync(path.join(companion, "origin.js"), "utf8");
...
assert.deepEqual(popupScripts, ["origin.js", "state.js", "popup.js"], "popup must load origin and shared state before popup runtime");
...
assert.match(background, /importScripts\("origin\.js", "state\.js"\)/);
assert.match(popup, /syncDailyPageContext/);
assert.match(manifest.content_scripts[0].js.join(" "), /origin\.js/);

const originContext = { globalThis: {}, URL };
vm.runInNewContext(origin, originContext);
const dailyOrigin = originContext.globalThis.MyfansDailyPageOrigin;
const resolveOrigin = (value) => JSON.parse(JSON.stringify(dailyOrigin.resolve(value)));
assert.deepEqual(resolveOrigin("http://localhost:3000/admin/myfans?media=1"), { baseUrl: "http://localhost:3000", approvedMediaId: "1", kind: "local" });
assert.deepEqual(resolveOrigin("http://localhost:3000/admin/myfans"), { baseUrl: "http://localhost:3000", approvedMediaId: "", kind: "local" });
assert.equal(dailyOrigin.resolve("http://localhost:3001/admin/myfans?media=1"), null, "stale port 3001 must not be accepted");
assert.deepEqual(resolveOrigin("https://hakkutsu-lab.com/admin/myfans?media=1"), { baseUrl: "https://hakkutsu-lab.com", approvedMediaId: "1", kind: "production" });
assert.deepEqual(resolveOrigin("https://amateur-lab.vercel.app/admin/myfans"), { baseUrl: "https://amateur-lab.vercel.app", approvedMediaId: "", kind: "production" });
assert.equal(dailyOrigin.resolve("http://localhost:3000/admin"), null);
assert.equal(dailyOrigin.resolve("http://localhost:3000/not-daily"), null);
...
assert.match(background, /dailyPageTabId/);
assert.match(background, /active: false/);
assert.match(background, /RECREATE_WORKER_TAB/);
assert.match(background, /SAVE_RESULT/);
assert.match(background, /waitForStatusNavigation/);
assert.match(background, /statusAnchorCount/);
assert.match(background, /observedStatusIds/);
assert.match(background, /authorCandidates/);
...
assert.match(background, /payload\?\.singleStatusRunId !== result\.singleStatusRunId/);
assert.match(background, /saveReached !== true/);
assert.match(background, /status: "SUCCEEDED"/);
assert.match(background, /status: "FAILED"/);
assert.match(popup, /crypto\.randomUUID\(\)/);
assert.match(popup, /ensureSingleStatusReady/);
assert.match(bridge, /myfans-single-status:state/);
assert.match(companionApi, /phase: "received"/);
assert.match(companionApi, /duplicate\/replayed/);
assert.match(companionApi, /saveReached: true/);
assert.match(companionApi, /single_status_latest.*runId/s);
assert.match(background, /httpReached/);
assert.match(background, /urlKind/);
assert.match(background, /failureDiagnostics/);
assert.match(companionApi, /collectionFailure/);
assert.match(companionApi, /failure:/);
assert.match(popup, /dailyPageTabId/);
...
const identity = { job_id: 36, collection_session_id: "session-a", run_token: "run-a", collector_version: "0.1.27" };
const activeJob = { id: 36, status: "running", collection_session_id: "session-a", collection_run_token: "run-a", collector_version: "0.1.27" };
assert.equal(companionState.isCurrentActiveRun({ running: true, status: "running", identity }, activeJob, "0.1.27"), true);
for (const status of ["completed", "cancelled", "failed"]) {
  assert.equal(companionState.isCurrentActiveRun({ running: true, status: "scheduled", identity }, { ...activeJob, status }, "0.1.27"), false);
}
assert.equal(companionState.isCurrentActiveRun({ running: true, status: "scheduled", identity }, { ...activeJob, collection_session_id: null }, "0.1.27"), false);
assert.equal(companionState.isCurrentActiveRun({ running: true, status: "scheduled", identity }, { ...activeJob, collection_run_token: null }, "0.1.27"), false);
assert.equal(companionState.isCurrentActiveRun({ running: true, status: "scheduled", identity: { ...identity, collector_version: "0.1.26" } }, activeJob, "0.1.27"), false);
assert.equal(companionState.sanitizeForDisplay({ running: true, status: "scheduled", identity }, { ...activeJob, status: "completed" }, "0.1.27").history, true);
...
assert.match(companionApi, /resolveExactMyfansCreator/);
...
assert.match(companionApi, /ensureExactProductForCreator/);
assert.match(companionApi, /const resolveBatchProduct = async/);
assert.match(companionApi, /await ensureExactProductForCreator\(finalMyfansUrl, resolvedCreatorId, approvedMediaId\)/);
assert.match(companionApi, /\.eq\("creator_id", creatorId\)\.eq\("x_post_url", postUrl\)/);
assert.match(companionApi, /quote_candidate_id: savedCandidateId/);
assert.match(companionApi, /cursor_after_order: cursorAfterOrder/);
assert.match(companionApi, /cycle_completed: cycleCompleted/);
assert.match(companionApi, /existing_canonical_url_then_creator_exact/);
assert.match(companionApi, /canonical_url_observed_import_then_creator_exact/);
assert.match(companionApi, /canonical_url_observed_title_missing/);
assert.match(companionApi, /matchedCreatorId/);
assert.match(companionApi, /observedMfcoUrl/);
assert.match(companionApi, /matchedOrCreatedProductId/);
...
assert.match(companionApi, /saveQuoteCandidate\(record, creatorMatch\.creatorId, existingCandidate\?\.id\)/);
