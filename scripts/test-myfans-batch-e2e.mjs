...
async function createHarness({ itemCount = 5, workerStopAfter = 0, retryItem = 0, jobStatus = "running", staleRunning = false } = {}) {
  ...
  const listeners = { alarm: null, runtimeMessage: null };
  ...
  const chrome = {
    runtime: { getManifest: () => ({ version: "0.1.37" }), onInstalled: { addListener: (fn) => { callbacks.runtimeInstalled = fn; } }, onMessage: { addListener: (fn) => { listeners.runtimeMessage = fn; } } },
...
const startMessage = await createHarness();
let startResponse;
let startResponseReturnedSynchronously = false;
const startResponsePromise = new Promise((resolve) => {
  startMessage.listeners.runtimeMessage({ type: "myfans_quote_refresh_start", settings: { baseUrl: "http://localhost", approvedMediaId: "1", batchSize: 5, queueLimit: 5 } }, {}, (response) => {
    startResponse = response;
    resolve(response);
  });
  startResponseReturnedSynchronously = startResponse !== undefined;
});
assert.equal(startResponseReturnedSynchronously, false, "start response must keep the MV3 message channel open until startup work finishes");
const started = await startResponsePromise;
assert.equal(started.ok, true, "popup start message completes successfully");
assert.equal(startMessage.job.status, "completed", "popup start path creates and completes exactly one fixture job");
assert.equal(startMessage.items.filter((item) => item.status === "success").length, 5, "popup start path reaches all fixture items");
